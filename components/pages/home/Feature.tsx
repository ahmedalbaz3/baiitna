...
const Feature = async () => {
  ...
  return (
    <section className="bg-white py-sm-section-py md:py-section-py">
        ...
        <div className="text w-full">
          ...
          <div className="w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mt-10 ">
            {steps.map((step, index) => (
              <div
                key={index}
                className={`relative font-semibold p-6 flex h-fit rounded-2xl  ${step.color}`}
              >
                <div className=" flex flex-col justify-between relative z-10">
                  <span className="pb-10 md:pb-21 w-full text-2xl md:text-5xl">
                    0{index + 1}
                  </span>
                  <h3 className="max-w-56.25 text-2xl md:text-[32px]">
                    {step.title}
                  </h3>
                </div>
                <Image
                  ...
                  width={400}
                  height={540}
                  className={`max-h-[90%] max-w-[60%] absolute top-2  md:top-6 ${
